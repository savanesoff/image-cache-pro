...
const createBucket = ({ name = 'test', lock = false, controller, } = {}) => {
...
const createRequest = ({ url = 'test-url', size = { width: 100, height: 100 }, bucket = createBucket(), listeners = {}, } = {}) => {
...
describe('Bucket', () => {
    ...
    describe('unregisterRequest', () => {
        ...
        it('should unregister request listeners', () => {
            ...
            request.off = vi.fn();
            bucket.unregisterRequest(request);
            expect(request.off).toHaveBeenCalledWith('loadstart', expect.any(Function));
    ...
    describe('getRamBytes', () => {
        ...
        it('should have correct compressed data', () => {
            const compressed = Array.from(bucket.requests).reduce((acc, request) => acc + request.image.bytes, 0);
            expect(bucket.getRamBytes().compressed).toBe(compressed);
        });
        it('should have correct uncompressed data', () => {
            const uncompressed = Array.from(bucket.requests).reduce((acc, request) => acc + request.image.bytesUncompressed, 0);
            expect(bucket.getRamBytes().uncompressed).toBe(uncompressed);
        });
        it('should have correct total data', () => {
            const total = Array.from(bucket.requests).reduce((acc, request) => acc + request.image.bytes + request.image.bytesUncompressed, 0);
    ...
    describe('getRamUnits', () => {
        ...
        it('should have correct compressed data', () => {
            const compressed = Array.from(bucket.requests).reduce((acc, request) => acc + request.image.bytes, 0) / UNITS[units];
            expect(bucket.getRamUnits().compressed).toBe(compressed);
        });
        it('should have correct uncompressed data', () => {
            const uncompressed = Array.from(bucket.requests).reduce((acc, request) => acc + request.image.bytesUncompressed, 0) / UNITS[units];
            expect(bucket.getRamUnits().uncompressed).toBe(uncompressed);
        });
        it('should have correct total data', () => {
            const total = Array.from(bucket.requests).reduce((acc, request) => acc + request.image.bytes + request.image.bytesUncompressed, 0) / UNITS[units];
    ...
    describe('getVideoBytes', () => {
        ...
        it('should have correct requested data', () => {
            const requested = Array.from(bucket.requests).reduce((acc, request) => acc + request.bytesVideo, 0);
        ...
        it('should have correct used data', () => {
            ...
            const used = Array.from(bucket.requests).reduce((acc, request) => acc + (request.rendered ? request.bytesVideo : 0), 0);
    ...
    describe('getVideoUnits', () => {
        ...
        it('should have correct requested data', () => {
            const requested = Array.from(bucket.requests).reduce((acc, request) => acc + request.bytesVideo, 0) / UNITS[units];
        ...
        it('should have correct used data', () => {
            ...
            const used = Array.from(bucket.requests).reduce((acc, request) => acc + (request.rendered ? request.bytesVideo : 0), 0) / UNITS[units];
