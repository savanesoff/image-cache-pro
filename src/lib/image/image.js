...
export class Img extends Loader {
    ...
    }, type = 'RGB', gpuDataFull = false, logLevel = 'error', name = 'Image', mimeType = 'unknown', ...props }) {
        super({
            headers,
            name,
            logLevel,
    ...
    #onRequestClear = (event) => {
        ...
        this.emit('render-request-removed', {
            ...
            bytes: this.gpuDataFull && this.renderRequests.size > 0
                ? 0
    ...
    isSizeLocked(callerRequest) {
        for (const request of this.renderRequests.values()) {
            if (request === callerRequest)
                continue;
            if ((this.gpuDataFull ||
    ...
    #onLoadEnd() {
        ...
        getImageData(this.xhr.response)
            .then(data => {
            this.log.verbose([
                'gotImageData',
                'data:',
