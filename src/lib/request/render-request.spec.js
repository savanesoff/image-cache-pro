...
const createImage = ({ url = 'test', listeners = {}, imageLoaded, } = {}) => {
    const image = new Img({ url });
    // @ts-expect-error - mock api
    image.on = vi.fn((type, handler) => {
...
const createBucket = ({ url = 'test', imageLoaded, listeners = {}, } = {}) => {
...
const createRequest = ({ url = 'test', size, bucket, imageLoaded, listeners, } = {}) => {
...
describe('RenderRequest', () => {
    ...
    it('should have bytesVideo of size assigned', () => {
        ...
        expect(request.image.getBytesVideo).toHaveBeenCalledWith(request.size, true);
    });
    ...
    describe('clear', () => {
        ...
        it('should remove image loadstart listener', () => {
            expect(request.image.off).toHaveBeenCalledWith('loadstart', expect.any(Function));
        });
        it('should remove image progress listener', () => {
            expect(request.image.off).toHaveBeenCalledWith('progress', expect.any(Function));
        });
        it('should remove image error listener', () => {
            expect(request.image.off).toHaveBeenCalledWith('error', expect.any(Function));
        });
    });
    describe('isLocked', () => {
        let request;
